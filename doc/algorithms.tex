...
series given a ball for the Fourier transfrom~$q$ of its complex
...
We may assume that $z$ lies in the fundamental domain given by
$|x| \leq 1/2$ and $|z| \geq 1$, which implies $y \geq \frac {\sqrt 3}{2}$.
...
algorithm. So that the algorithm really obtains an upper bound
...
is strictly increasing for $0 < \alpha < 1/4$ with a maximum of
...
\label {eq:qfromzcm}
\begin {aligned}
k_a & \leq 16 \cdot 2^{\Exp (\appro y)} + 10 \\
k_b & \leq 16 \cdot 2^{\Exp (\appro y)} + 38
\end {aligned}
\end {equation}
...
$k_a, k_b \leq 2^{20}$, which corresponds to a loss of precision bounded
by about $20$~bits.
